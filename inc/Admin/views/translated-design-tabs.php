...
<script>
/**
 * Toggle Translated Design Tab Group content
 */
function toggleTranslatedDTGroup(groupId) {
    const content = document.getElementById('translated-dt-content-' + groupId);
    const toggle = document.querySelector('.translated-dt-toggle-' + groupId);
...
/**
 * Navigate to Tasks → Design Tabs and expand the specified group
 */
function goToEditDesignTabs(groupId) {
    // 1. Switch to Tasks view
    if (typeof switchView === 'function') {
        switchView('tasks');
    }
    
    // 2. Wait for view to load, then switch to Design Tabs tab
    setTimeout(() => {
        if (typeof switchTasksTab === 'function') {
            switchTasksTab('design-tabs');
        }
        
        // 3. Wait for tab to load, then expand the group
        setTimeout(() => {
            const groupCard = document.getElementById('dt-group-card-' + groupId);
            if (groupCard) {
                groupCard.scrollIntoView({ behavior: 'smooth', block: 'center' });
                // Click to expand
                if (typeof toggleDesignTabGroup === 'function') {
                    toggleDesignTabGroup(groupId);
                }
            }
        }, 200);
    }, 100);
