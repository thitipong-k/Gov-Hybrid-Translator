...
<script>
// === Radio Button Visual Handler ===
// จัดการ visual state สำหรับ radio buttons ทั้งหมด
function initRadioHandlers() {
    // Handler สำหรับ switcher_type
    document.querySelectorAll('input[name="switcher_type"]').forEach(radio => {
        radio.addEventListener('change', function() {
            // ลบ active state จากทุก labels
            document.querySelectorAll('input[name="switcher_type"]').forEach(r => {
                const label = r.closest('label');
                if (label) {
                    label.classList.remove('border-gov-500', 'bg-gov-50');
                    label.classList.add('border-gray-200');
                }
            });
            // เพิ่ม active state ให้ label ที่เลือก
            const selectedLabel = this.closest('label');
            if (selectedLabel) {
                selectedLabel.classList.remove('border-gray-200');
                selectedLabel.classList.add('border-gov-500', 'bg-gov-50');
            }
        });
    });

    // Handler สำหรับ floating_position
    document.querySelectorAll('input[name="floating_position"]').forEach(radio => {
        radio.addEventListener('change', function() {
            // ลบ active state จากทุก labels
            document.querySelectorAll('input[name="floating_position"]').forEach(r => {
                const label = r.closest('label');
                if (label) {
                    label.classList.remove('border-gov-500', 'bg-gov-50');
                    label.classList.add('border-gray-200');
...
// แสดง/ซ่อนตัวเลือก Floating Button ตามสถานะ checkbox
document.getElementById('placement_floating')?.addEventListener('change', function() {
    const settingsPanel = document.getElementById('floating-button-settings');
    if (settingsPanel) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Floating settings visibility
    const floatingCheckbox = document.getElementById('placement_floating');
    const settingsPanel = document.getElementById('floating-button-settings');
    if (floatingCheckbox && settingsPanel) {
        settingsPanel.style.display = floatingCheckbox.checked ? 'block' : 'none';
    }
    updateFloatingPreview();
});

// === Update Preview ===
// อัพเดท preview เมื่อเปลี่ยนตำแหน่งหรือระยะห่าง
function updateFloatingPreview() {
    const preview = document.getElementById('floating-preview-button');
    if (!preview) return;
    
    const position = document.querySelector('input[name="floating_position"]:checked')?.value || 'bottom-right';
    const marginX = document.querySelector('input[name="floating_margin_x"]')?.value || 20;
    const marginY = document.querySelector('input[name="floating_margin_y"]')?.value || 20;
    ...
    if (position.includes('top')) {
        preview.style.top = marginY + 'px';
    } else {
        preview.style.bottom = marginY + 'px';
    }
    
    if (position.includes('left')) {
        preview.style.left = marginX + 'px';
    } else {
        preview.style.right = marginX + 'px';
    }
}

// Listen for margin changes
document.querySelector('input[name="floating_margin_x"]')?.addEventListener('input', updateFloatingPreview);
document.querySelector('input[name="floating_margin_y"]')?.addEventListener('input', updateFloatingPreview);
</script>
