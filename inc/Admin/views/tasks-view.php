...
<div id="view-tasks" class="view-section hidden space-y-6">
    ...
    function updateButtonText(postId) {
        ...
        if (input && btn) {
            btn.innerText = input.value.trim() ? 'Save' : 'Translate';
        }
    }

    /**
     * ย้ายแถวที่แปลเสร็จออกจากตาราง พร้อม animation
     * @param {HTMLElement} row - แถวที่ต้องการย้าย
     * @param {string} type - 'page' หรือ 'post'
     */
    function moveRowToTranslated(row, type) {
        ...
        setTimeout(() => {
            ...
            const tabBtn = document.getElementById('tab-tasks-btn-' + type + 's');
            if (tabBtn) {
            ...
            const tableBody = document.querySelector('#tab-tasks-content-' + type + 's tbody');
            if (tableBody && tableBody.querySelectorAll('tr').length === 0) {
                const emptyRow = document.createElement('tr');
                emptyRow.innerHTML = '<td colspan="4" class="px-6 py-4 text-center text-gray-500">All ' + type + 's have been translated!</td>';
    ...
    function savePageTranslation(pageId) {
        ...
        const langSelect = document.getElementById('lang-select-page-' + pageId);
        ...
        if (!translation.trim()) {
            showNotification('Please enter translated title', 'error');
            return;
        }
    ...
    /**
     * Translate post ไปยังภาษาที่เลือก
     * ใช้ clone_to_language() แทน clone_to_english()
     */
    function translatePost(postId) {
        const input = document.getElementById('post-trans-' + postId);
        const langSelect = document.getElementById('lang-select-post-' + postId);
